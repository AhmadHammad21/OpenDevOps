...
import { Plus, LayoutDashboard, MessageSquare, Terminal, GitBranch, Users, Settings, LogOut, Radio, Trash2, ChevronDown } from 'lucide-react';
...
export default function Sidebar({ sessions, hasMore, currentSessionId, onNew, onSwitch, onDelete, onLoadMore }: Props) {
  ...
  return (
    <>
            ...
            <>
              {sessions.map(s => (
                <div
                  ...
                  className={cn(
                    'px-3.5 py-[7px] cursor-pointer relative transition-all duration-100 border-l-2 select-none',
      ...
      {/* Right-click context menu — rendered outside aside to avoid clipping */}
      {contextMenu && (
        <div
          ref={menuRef}
          className="fixed z-50 bg-white dark:bg-[#1E1E24] border border-gray-200 dark:border-[#27272F] rounded-lg shadow-lg py-1 min-w-[140px]"
