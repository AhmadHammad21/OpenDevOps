...
from agent.llm.detectors import ALL_DETECTORS
...
def get_backend_info() -> LlmBackendInfo:
    ...
    if api_base:
        return LlmBackendInfo(
            source="custom", model=model, display_name="Custom endpoint",
            provider="custom", configured=True, detail=api_base,
        )
    ...
    if os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN"):
        ...
        return LlmBackendInfo(
            source=source, model=model, display_name=label,
            provider=_provider_of(model), configured=True, detail=_strip_date(model),
        )

    # OpenRouter key
    if settings.openrouter_api_key and "openrouter" in model:
        return LlmBackendInfo(
            source="openrouter", model=model, display_name="OpenRouter",
            provider="openrouter", configured=True, detail=_strip_date(model),
        )
    ...
    if explicit_model:
        ...
        if has_key:
            return LlmBackendInfo(
                source=provider, model=model, display_name=provider.title() + " (via .env)",
                provider=provider, configured=True, detail=_strip_date(model),
            )
    ...
    return LlmBackendInfo(
        source="default", model=model, display_name="Not configured",
        provider=_provider_of(model), configured=False,
