...
def _build_message(
    ...
    header = (
        "🧪 <b>[TEST] OpenDevOps — Investigation Complete</b>"
        if is_test
        else "🔍 <b>OpenDevOps — Investigation Complete</b>"
    )
    ...
    if services:
        lines += [
            f"<b>Services affected:</b> {', '.join(f'<code>{s}</code>' for s in services)}",
            "",
        ]
...
async def post_failed_investigation(
    ...
    header = (
        "🧪 <b>[TEST] OpenDevOps — Investigation Failed</b>"
        if is_test
        else "⚠️ <b>OpenDevOps — Investigation Failed</b>"
    )
    text = (
        f"{header}\n\n"
