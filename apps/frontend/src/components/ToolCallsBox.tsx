...
export default function ToolCallsBox({ calls, streaming }: Props) {
  ...
  return (
    <div className="shrink-0 border border-gray-200 dark:border-[#1E222B] rounded-[7px] overflow-hidden text-xs">
      <div
        className="flex items-center gap-2 px-2.5 py-[5px] bg-gray-100 dark:bg-[#15181F] cursor-pointer hover:bg-gray-200/60 dark:hover:bg-[#1E222B] transition-colors select-none"
        onClick={() => setOpen(o => !o)}
      >
        <Wrench size={11} className="text-indigo-500 dark:text-[#00A3FF] shrink-0" />
        <span className="text-indigo-500 dark:text-[#00A3FF] font-semibold font-mono">{calls.length}</span>
      ...
      {open && (
        <div className="flex flex-col">
          {calls.map((tc, i) => (
            <div key={i} className="border-t border-gray-200 dark:border-[#1E222B] p-2.5 flex flex-col gap-2">
          ...
          {streaming && (
            <div className="border-t border-gray-200 dark:border-[#1E222B] px-2.5 py-2 flex items-center gap-2 text-gray-400 dark:text-[#64748B]">
