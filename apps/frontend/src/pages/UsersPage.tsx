...
function RoleBadge({ role }: { role: string }) {
  return role === 'admin' ? (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-indigo-600 dark:text-[#00A3FF] bg-indigo-50 dark:bg-[#04103A] border border-indigo-200 dark:border-[#0E4FA6] rounded px-1.5 py-[2px]">
      <Shield size={10} /> admin
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-gray-500 dark:text-[#94A3B8] bg-gray-100 dark:bg-[#1E222B] border border-gray-200 dark:border-[#2A2F3A] rounded px-1.5 py-[2px]">
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-[#000000] min-h-0">
      ...
      <div className="bg-white dark:bg-[#0A0C10] border-b border-gray-200 dark:border-[#1E222B] px-7 py-[14px]">
        <div className="text-[16px] font-bold text-gray-900 dark:text-[#E4E1EA] tracking-[-0.02em]">Team</div>
      ...
      <div className="px-7 py-6 max-w-[760px]">
        <div className="bg-white dark:bg-[#0A0C10] border border-gray-200 dark:border-[#1E222B] rounded-lg overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          {/* Table header */}
          <div className="px-4 py-[11px] border-b border-gray-200 dark:border-[#1E222B] flex items-center justify-between bg-gray-50 dark:bg-[#15181F]">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[13px] font-medium text-gray-500 dark:text-[#94A3B8] hover:text-gray-700 dark:hover:text-[#E4E1EA] px-2.5 py-[5px] border border-gray-300 dark:border-[#2A2F3A] rounded-[5px] bg-white dark:bg-[#0A0C10] shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-colors"
            >
          ...
          {loading ? (
            ...
            users.map((u, i) => (
              <div key={u.id} className={`flex items-center gap-2 px-4 py-[10px] ${i < users.length - 1 ? 'border-b border-gray-200 dark:border-[#1E222B]' : ''}`}>
                <div className="flex-1 min-w-0">
                  <div className="text-[14px] font-medium text-gray-900 dark:text-[#E4E1EA] truncate">{u.name}</div>
                ...
                <div className="w-24">
                  {currentUser?.id === u.id ? (
                    ...
                    <select
                      ...
                      className="text-[12px] font-medium bg-white dark:bg-[#0A0C10] border border-gray-300 dark:border-[#2A2F3A] rounded-[5px] px-1.5 py-[3px] text-gray-700 dark:text-[#CBD5E1] outline-none focus:border-indigo-500 dark:focus:border-[#00A3FF] transition-colors cursor-pointer"
                    >
                ...
                <div className="w-8 flex justify-end">
                  {currentUser?.id !== u.id && (
                    <button
                      ...
                      className="p-[3px] rounded hover:bg-gray-100 dark:hover:bg-[#1E222B] text-gray-400 dark:text-[#64748B] hover:text-red-500 dark:hover:text-[#F87171] transition-colors"
                    >
                      <Trash2 size={13} />
                    </button>
                  )}
          ...
          {showAdd && (
            <form onSubmit={handleAdd} className="border-t border-gray-200 dark:border-[#1E222B] px-4 py-4 bg-gray-50 dark:bg-[#15181F] flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={addName} onChange={e => setAddName(e.target.value)} required placeholder="Full name"
                    className="w-full text-[13px] text-gray-900 dark:text-[#E4E1EA] bg-white dark:bg-[#0A0C10] border border-gray-300 dark:border-[#2A2F3A] rounded-[5px] px-2 py-1.5 outline-none focus:border-indigo-500 dark:focus:border-[#00A3FF] transition-all" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={addRole} onChange={e => setAddRole(e.target.value as 'admin' | 'user')}
                    className="w-full text-[12px] bg-white dark:bg-[#0A0C10] border border-gray-300 dark:border-[#2A2F3A] rounded-[5px] px-2 py-1.5 text-gray-900 dark:text-[#E4E1EA] outline-none focus:border-indigo-500 dark:focus:border-[#00A3FF] transition-all cursor-pointer">
                    <option value="user">user</option>
                    <option value="admin">admin</option>
                  </select>
                </div>
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={addBusy}
                  className="flex items-center gap-1.5 text-[12px] font-medium text-white bg-indigo-500 dark:bg-[#00A3FF] hover:bg-indigo-600 dark:hover:bg-[#0086D6] disabled:opacity-50 px-3 py-1.5 rounded-[5px] transition-colors">
                  <Plus size={12} /> {addBusy ? 'Creating…' : 'Create user'}
                </button>
                <button type="button" onClick={() => setShowAdd(false)}
                  className="text-[12px] font-medium text-gray-500 dark:text-[#94A3B8] hover:text-gray-700 dark:hover:text-[#E4E1EA] px-3 py-1.5 rounded-[5px] transition-colors">
                  Cancel
                </button>
              </div>
            </form>
