...
def test_max_receive_count_constant():
    ...
    from agent.event_infra import MAX_RECEIVE_COUNT
...
def test_dlq_queue_name_constant():
    from agent.event_infra import DLQ_QUEUE_NAME, QUEUE_NAME
...
async def test_consumer_loop_skips_delete_on_bad_json(monkeypatch):
    ...
    with (
        patch("agent.event_consumer.get_runtime_sqs_queue_url", return_value="https://fake-url"),
        patch("agent.event_consumer.get_runtime_aws_region", return_value="us-east-1"),
...
async def test_consumer_loop_skips_delete_on_processing_error(monkeypatch):
    ...
    with (
        patch("agent.event_consumer.get_runtime_sqs_queue_url", return_value="https://fake-url"),
        patch("agent.event_consumer.get_runtime_aws_region", return_value="us-east-1"),
        patch("agent.event_consumer._process_event", side_effect=fake_process_event),
        patch("boto3.Session") as mock_session,
    ):
        mock_session.return_value.client.return_value = mock_sqs
        from agent.event_consumer import event_consumer_loop
